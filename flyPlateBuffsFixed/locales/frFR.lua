local _, fPB = ...

if GetLocale() ~= "frFR" then
  return
end

--You can help with translation at https://wow.curseforge.com/projects/flyPlateBuffsFixed/localization

local L = fPB.L

L["Add new spell to list"] = "Ajouter nouveau sort"
L["All"] = "Tout"
L["Allies"] = "Alliés"
L["Also will show duration if OmniCC installed, regardless of the previous option"] =
  "Affichera aussi la durée si Omnicc est installé, quelle que soit la précédente option"
L["Always"] = "Toujours"
L["Always show icons with full opacity and size"] = "Toujours afficher pleinement les icônes en grand"
