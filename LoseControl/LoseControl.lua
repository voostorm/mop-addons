...
local spellIds = {
	...
	[129888]  = "Silence",		-- Solar Beam
	[129889]  = "Silence",		-- Solar Beam
...
local function CreateAliasForPartyFrames()
	LCPartyMemberFrame1 = PartyMemberFrame1
	LCPartyMemberFrame2 = PartyMemberFrame2
	LCPartyMemberFrame3 = PartyMemberFrame3
	LCPartyMemberFrame4 = PartyMemberFrame4
end
...
function LoseControl:GROUP_ROSTER_UPDATE()
	...
	if (enabled and IsInRaid() and (frame.anchor == "Blizzard") and (self.useCompactPartyFrames) and strfind(self.fakeUnitId or self.unitId, "party") and tonumber(C_CVar.GetCVar("useCompactPartyFrames")) > 0 and UnitExists(self.unitId)) then
...
function LoseControl:CheckStatusPartyFrameChange(value)
	if (value == nil) then value = tonumber(C_CVar.GetCVar("useCompactPartyFrames")) end
...
if type(GetAddOnMetadata) == "function" then
    notes = GetAddOnMetadata(addonName, "Notes-" .. GetLocale()) or GetAddOnMetadata(addonName, "Notes") or ""
end
...
function OptionsPanelRefresh() -- This method will run when the Interface Options frame calls its OnShow function and after defaults have been applied via the panel.default method described above.
...
for _, v in ipairs({ "player", "pet", "target", "targettarget", "focus", "focustarget", "party", "arena", "raid", "nameplate" }) do
	...
	function AnchorDropDown:OnClick()
		...
		for _, unitId in ipairs(frames) do
			...
			if (unitId ~= "partyplayer") then
				...
				if ((self.value ~= "None" and self.value ~= "Blizzard" and anchors[self.value].partyplayer ~= nil) or (self.value == "Blizzard" and tonumber(C_CVar.GetCVar("useCompactPartyFrames")) > 0)) then
					frame.anchor = self.value
				else
					frame.anchor = "None"
				end
			...
			if v == "raid" and frame.anchor == "BlizzardRaidFrames" then
				MainHookCompactRaidFrames()
			elseif v == "party" and frame.anchor == "Blizzard" and tonumber(C_CVar.GetCVar("useCompactPartyFrames")) > 0 then
				MainHookCompactRaidFrames()
			end
	...
	SizeSlider.Func = function(self, value)
		...
		for _, frame in ipairs(frames) do
			if (v ~= "party" or tonumber(C_CVar.GetCVar("useCompactPartyFrames")) > 0 or not(LoseControlDB.showPartyplayerIcon) or LoseControlDB.frames[frame].anchor ~= "Blizzard" or frame == "partyplayer") then
	...
	OptionsPanelFrameSetDefault = OptionsPanelSetDefault
	local function OptionsPanelFrameRefresh()
--		print(format("OptionsPanel refresh for unitid %s called", v))
		local unitId = v
	...
	EventRegistry:RegisterCallback("Settings.CategoryChanged", function(OwnerID, category)
		if category.name == OptionsPanelFrame.name and CategoryLoaded == false then
	       		OptionsPanelFrameRefresh()
		end
	end)
	OptionsPanelFrame:RegisterEvent("ADDON_LOADED")
	local subcategory
	subcategory = Settings.RegisterCanvasLayoutSubcategory(LoseControlCategory, OptionsPanelFrame, OptionsPanelFrame.name)
end
...
SlashCmdList[addonName] = function(cmd)
	...
	if SlashCmd[args[1]] then
		...
		LCCategoryID = LoseControlCategory:GetID()
		Settings.OpenToCategory(LCCategoryID)
	end
end
