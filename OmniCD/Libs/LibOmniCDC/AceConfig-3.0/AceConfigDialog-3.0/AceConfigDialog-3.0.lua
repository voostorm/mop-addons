...
local OmniCDC = LibStub("LibOmniCDC")

--[[ s r
local MAJOR, MINOR = "AceConfigDialog-3.0", 92
]]
local MAJOR, MINOR = "AceConfigDialog-3.0-OmniCDC", 2
-- e
...
AceConfigDialog.tooltip = AceConfigDialog.tooltip or CreateFrame("GameTooltip", "AceConfigDialogTooltip-OmniCDC", UIParent, BackdropTemplateMixin and "GameTooltipTemplate, BackdropTemplate" or "GameTooltipTemplate")
--if select(4, GetBuildInfo()) > 90100 then -- 9.1.5 fix > Blizzard added this for classic era
--if WOW_PROJECT_ID ~= WOW_PROJECT_BURNING_CRUSADE_CLASSIC then -- added in TBC Anniversary
...
local function CleanUserData(widget, event)
	...
	if widget.type == "TreeGroup-OmniCDC" then
	-- e
	...
	if widget.type == "TabGroup-OmniCDC" then
	-- e
		widget:SetTabs(nil)
...
-- s b
local noRefresh = {
	["InlineGroupList2Slider-OmniCDC"] = true,
	["InlineGroupListCheckBox-OmniCDC"] = true,
	["InlineGroupList2CheckBox-OmniCDC"] = true,
}
		...
		if not hidden then
			if v.type == "group" then
				if inline or pickfirstset(v.dialogInline,v.guiInline,v.inline, false) then
					...
					if v.dialogControl == "InlineGroupList2-OmniCDC" then
						GroupContainer = gui:Create("InlineGroupList2-OmniCDC")
						GroupContainer:SetTitle(v.arg) -- class color bg
						GroupContainer.width = "fill"
						GroupContainer:SetLayout("Flow-NowrapFix-OmniCDC")
						container:AddChild(GroupContainer)
						FeedOptions(appName,options,GroupContainer,rootframe,path,v,true)
					else
						if name and name ~= "" then
							GroupContainer = gui:Create("InlineGroup-OmniCDC")
							GroupContainer:SetTitle(name or "")
						else
							GroupContainer = gui:Create("SimpleGroup-OmniCDC")
						end
				...
				if v.type == "execute" then
					...
					control = CreateControl(v.dialogControl or v.control, iconControl and "Icon-OmniCDC" or "Button-OmniCDC") -- for 'Profiles'
					-- e
					if iconControl then
						if not width then
				...
				elseif v.type == "input" then
					...
					control = CreateControl(v.dialogControl or v.control, v.multiline and "MultiLineEditBox" or "EditBox-OmniCDC") -- for 'Profiles'
					-- e
				...
				elseif v.type == "toggle" then
					...
					control = CreateControl(v.dialogControl or v.control, "CheckBox-OmniCDC") -- for 'Profiles'
					if v.dialogControl then
						if  v.dialogControl == "InlineGroupList2CheckBox-OmniCDC" then -- has img
							control:SetLabel(name)
				...
				elseif v.type == "range" then
					...
					control = CreateControl(v.dialogControl or v.control, "Slider-OmniCDC")
					if v.dialogControl == "InlineGroupList2Slider-OmniCDC" then
						control:SetSliderValues(v.softMin or v.min or 0, v.softMax or v.max or 100, v.bigStep or v.step or 0)
					else
						control:SetLabel(name)
						control:SetSliderValues(v.softMin or v.min or 0, v.softMax or v.max or 100, v.bigStep or v.step or 0)
				...
				elseif v.type == "select" then
					...
					if v.style == "radio" then
						...
						control = gui:Create("InlineGroup-OmniCDC")
						-- e
						...
						for _, value in ipairs(sorting) do
							...
							local radio = gui:Create("CheckBox-OmniCDC")
							-- e
							radio:SetLabel(text)
						...
						control = CreateControl(v.dialogControl or v.control, "Dropdown-OmniCDC") -- for 'Profiles'
						-- e
				...
				elseif v.type == "multiselect" then
					...
					if control then
						...
						if controlType == "InlineGroupList-OmniCDC" then
							control:SetLayout("Flow-NowrapFix-OmniCDC") -- fixed width
							...
							for s = 1, #valuesort do
								...
								local text = values[value]
								local check = gui:Create("InlineGroupListCheckBox-OmniCDC")
								check:SetLabel("")
						...
						for s = 1, #valuesort do
							...
							local check = gui:Create("CheckBox-OmniCDC")
							-- e
							check:SetLabel(text)
							check:SetUserData("value", value)
							check:SetUserData("text", text)
				...
				elseif v.type == "description" then
					...
					if fontSize == "medium" then
						control:SetFontObject(_G["GameFontHighlight-OmniCDC"])
					elseif fontSize == "large" then
						control:SetFontObject(GameFontHighlightLarge) -- TODO:
					else -- small or invalid
						control:SetFontObject(_G["GameFontHighlightSmall-OmniCDC"])
					end
...
function AceConfigDialog:FeedGroup(appName,options,container,rootframe,path, isRoot)
	...
	if (not (hasChildGroups and not inline)) or (grouptype ~= "tab" and grouptype ~= "select" and (parenttype == "tree" and not isRoot)) then
		...
		if container.type ~= "InlineGroup-OmniCDC" and container.type ~= "SimpleGroup-OmniCDC" then
			scroll = gui:Create("ScrollFrame-OmniCDC")
			local opt = path[#path-1]
			scroll:SetLayout(opt and opt:match("^list_%w+$") and "Flow-Nopadding-OmniCDC" or "flow" )
			scroll.width = "fill"
			scroll.height = "fill"
			container:SetLayout("fill")
			container:AddChild(scroll)
	...
	if hasChildGroups and not inline then
		...
		if grouptype == "tab" then
			...
			local tab = gui:Create("TabGroup-OmniCDC")
			-- e
		...
		elseif (parenttype ~= "tree") or isRoot then
			...
			local tree = gui:Create("TreeGroup-OmniCDC")
			-- e
