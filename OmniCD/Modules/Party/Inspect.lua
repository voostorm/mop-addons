...
local GetSelectedTalentData = (E.postDF and function(info, unit, isInspect)
...
end) or (E.isBCC and E.TocVersion >= 20505 and function(info, unit, isInspect)
	local list
	if not isInspect then
		list = { CM.SERIALIZATION_VERSION, info.spec, "^T" }
	end

	local talentGroup = C_SpecializationInfo.GetActiveSpecGroup(isInspect, nil)
	...
	for tabIndex = 1, 3 do
		for i = 1, MAX_NUM_TALENTS do
			...
			talentInfoQuery.isInspect = isInspect
			talentInfoQuery.isPet = nil
			talentInfoQuery.groupIndex = talentGroup
			local talentInfo = C_SpecializationInfo.GetTalentInfo(talentInfoQuery)
			if talentInfo and talentInfo.rank > 0 then
				local talentRankIDs = E.talentNameToRankIDs[talentInfo.name]
				if talentRankIDs then
					if type(talentRankIDs[1]) == "table" then
						for _, t in pairs(talentRankIDs) do
							local talentID = t[talentInfo.rank]
							if talentID then
								info.talentData[talentID] = true
								if list then list[#list + 1] = talentID end
							end
						...
						local talentID = talentRankIDs[talentInfo.rank]
						if talentID then
							info.talentData[talentID] = true
							if list then list[#list + 1] = talentID end
						end
